...
import { activityLabels, goalLabels } from '../../services/profile';
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 128 }]}>
        <View style={styles.profileHead}><View style={styles.avatar}><Ionicons name="person-outline" size={30} color={colors.primary} /></View><View><AppText weight="bold" size={24}>{user?.first_name || 'Your profile'}</AppText><AppText style={styles.muted}>Health Profile</AppText></View><TouchableOpacity onPress={() => router.push('/edit-profile')} style={styles.edit}><Ionicons name="create-outline" size={19} color={colors.primary} /></TouchableOpacity></View>
        {saved ? <AppText style={styles.saved}>Profile saved successfully.</AppText> : null}
        ...
        {[{ title: 'Body metrics', detail: profile?.age === null || profile?.age === undefined ? 'Complete your profile' : `${profile.age} years  ·  ${profile.height ?? '—'} cm  ·  ${profile.weight ?? '—'} kg`, icon: 'body-outline', color: colors.blue }, { title: 'Goals', detail: goalLabels[profile?.goal] || 'Not added yet', icon: 'flag-outline', color: colors.mint }, { title: 'Activity', detail: activityLabels[profile?.activityLevel] || 'Not added yet', icon: 'fitness-outline', color: colors.lavender }, { title: 'Achievements', detail: '3 milestones unlocked', icon: 'ribbon-outline', color: colors.amber }].map((item) => <View style={styles.item} key={item.title}><View style={[styles.itemIcon, { backgroundColor: item.color }]}><Ionicons name={item.icon} size={19} color={colors.text} /></View><View style={{ flex: 1 }}><AppText weight="semibold">{item.title}</AppText><AppText style={styles.muted}>{item.detail}</AppText></View><Ionicons name="chevron-forward" size={17} color={colors.tertiaryText} /></View>)}
        <PrimaryButton title="Sign Out" onPress={handleLogout} loading={loggingOut} disabled={loggingOut} style={styles.logout} />
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  logout: { marginTop: 18 },
  saved: { color: colors.success, fontSize: 12, marginTop: 14 },
});
